...
def build_work(w, hall, siblings):
    ...
    if w.get("source") == "commons":
        ...
        # Коли автор кадру не встановлений, казати «права належать автору»
        # неточно: ми не знаємо, кому саме. Тоді пишемо як є.
        unknown = "невідом" in (w.get("author") or "").lower()
        facts += [("надіслано", "через форму /submit/"),
                  ("права", "автор не встановлений; кадр надіслано читачем архіву"
                            if unknown else
                            "належать автору; надано для показу в музеї")]
    else:
        facts += [("підпис", "авторський" if w["titled_by_author"] else "за розділом архіву"),
                  ("права", "авторський архів 062.dn.ua")]
    if w.get("note"):
        # Зауваження куратора досі жило лише в каталозі й не доходило до
        # відвідувача, хоча пишеться саме заради нього.
        facts += [("зауваження", esc(w["note"]))]
